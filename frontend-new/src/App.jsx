...
export default function App() {
  ...
  const { phase, history, year, cycle, llmCommentary, llmLoading } = sim.gameState

  if (phase === 'entry')       return <EntryPage onStart={sim.startGame} />
  if (phase === 'game')        return <GamePage sim={sim} />
  if (phase === 'report')      return <CycleReport history={history} year={year} cycle={cycle} onViewDetails={sim.dismissReport} llmCommentary={llmCommentary} llmLoading={llmLoading} />
