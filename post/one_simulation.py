...
decision_df = pd.DataFrame({
    ...
    'house_migration_amount': [0, 0, 50],
    'dam_levee_construction_cost': [0.0, 1.0, 1.0],
    'paddy_dam_construction_cost': [5.0, 5.0, 5.0],
    'capacity_building_cost': [10.0, 10.0, 10.0],
    'agricultural_RnD_cost': [5.0, 5.0, 0.0],
...
def plot_with_iqr(ax, df, indicator):
    ...
    elif indicator == 'Crop Yield':
        ax.set_ylim(0, 5500)
        ax.legend(loc='lower left')
    elif indicator == 'Municipal Cost':
        ax.set_ylim(0, 5e8)
        ax.legend(loc='lower left')


# --- 描画・保存（1×2, 凡例右） ---
indicators = ['Flood Damage', 'Ecosystem Level', 'Crop Yield', 'Municipal Cost']
fig, axes = plt.subplots(1, 4, figsize=(32, 4.5), sharex=True)
